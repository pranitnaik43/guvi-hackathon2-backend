...
const service = {
  async find(req, res) {   //get all cart products for the user
    const data = await db.users.findOne({ _id: new ObjectId(req.user._id) });   //find the data for the user
    console.log(data);
    res.send(data.cart);
  },
  async insert(req, res) {  //add product id to the cart of the user
  ...
  async delete(req,res) {   //remove product id from the cart of the user
